...
bool VESCDevice::sendCmd(const std::string_view& cmd, std::chrono::milliseconds delay_after_send)
{
...
bool VESCDevice::pktProcess(Comm* comm, const VESC::BoardId src_id, std::shared_ptr<VESC::Packet>& pkt)
{
  bool r = false;
  if(_wait_for_pkt_id == pkt->id)
  {
    //spdlog::debug("[VESCDevice::pktProcess][{}<={}] Wait is over for Packet {}", id, src_id, pkt->id);
  ...
  {
    //spdlog::debug("[VESCDevice::pktProcess][{}<={}] Got a handler for Packet {}", id, src_id, pkt->id);
    r = _pkt_handlers[pkt->id](comm, src_id, pkt);
  }

  //Only output debug if this is a Device
  if(_host && !r)
    spdlog::debug("[VESCDevice::pktProcess][{}<={}] Unhandled Packet {}", id, src_id, pkt->id);
  return r;
}
