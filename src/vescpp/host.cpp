...
void VESCHost::scanCAN(bool add_devices, std::chrono::milliseconds timeout_ms)
{
  if(add_devices)
  {
    std::vector<std::pair<VESC::BoardId, VESC::HwTypeId>> out;
    spdlog::debug("[{0}/0x{0:02X}] Scan CAN and add devices", this->id);
    VESCpp::scanCAN(timeout_ms, [this_id=id,&out](VESC::BoardId id, VESC::HwTypeId typ)
    {
